...
if __name__ == "__main__":
    ...
    # Run MILP
    start_time = time.time()
    routes_milp, cost_milp = solve_vrp_milp(clients, trucks, gruz, dist)
    ...
    # Визуализация маршрутов MILP
    print("\n=== Результаты MILP ===")
    if routes_milp is not None:
        plt.figure()
        plot_routes(x, y, routes_milp, cost_milp, gruz, clients, trucks['m'])
    else:
        print("Не удалось построить маршруты MILP для визуализации.")
    ...
    # Вывод результатов MILP
    print("\n=== Результаты MILP ===")
    if routes_milp is not None:
        print(f"Время выполнения: {milp_time:.2f} секунд")
        print(f"Суммарный перевозимый груз: {sum(gruz):.1f} кг")
        print(f"Общая потребность топлива: {cost_milp:.1f} литров")
        co2_milp = cost_milp * 2.7
        print(f"Общие выбросы CO₂: {co2_milp:.1f} кг")
        print("Лучшие маршруты для каждого грузовика:")
        
        truck_loads_milp = np.zeros(trucks['m'])
        truck_clients_milp = np.zeros(trucks['m'])
        for truck in range(trucks['m']):
            print(f"Грузовик {truck+1}: {routes_milp[truck]}")
            ind = np.array(routes_milp[truck])
            ind = ind[ind != 0]  # Убираем депо
            truck_loads_milp[truck] = sum(gruz[ind])
            truck_clients_milp[truck] = len(ind)
        
        print("Загрузка грузовиков:", truck_loads_milp)
        print("Клиентов на грузовик:", truck_clients_milp)
        
        # Расчет пробега и расхода топлива для MILP
        truck_dist_milp, truck_fuel_milp = plot_bar_dlina_fuels(trucks['m'], routes_milp, dist, trucks['fuel'], txt)
        print("Пробег грузовиков, км:", truck_dist_milp)
        print("Расход топлива, литры:", truck_fuel_milp)
        
        # Расчет загрузки и клиентов для MILP
        truck_loads_milp, truck_clients_milp = plot_bar_client_gruz(trucks['m'], routes_milp, gruz, txt)
        print("Загрузка грузовиков:", truck_loads_milp)
        print("Клиентов на грузовик:", truck_clients_milp)
    else:
        print("Не удалось найти решение с помощью MILP.")